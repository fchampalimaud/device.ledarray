...
namespace Harp.LedArray.Design.Adapters;
...
public class RgbRegisterAdapter : ReactiveObject
{
    ...
    public RgbRegisterAdapter(string registerKey, int colorCount, bool isWritable, RgbKind kind)
    {
        ...
        // Subscribe to color changes in each item
        foreach (var item in Colors)
        {
            item.WhenAnyValue(x => x.Color)
                .Skip(1)
                .Subscribe(_ => OnColorsChanged());
        }
    }
    ...
    /// <summary>
    /// Converts the adapter's colors to a struct value.
    /// Assumes the struct has fields/properties named Red0, Green0, Blue0, etc.
    /// </summary>
    public object ToStructValue(Type structType)
    {
        object instance = Activator.CreateInstance(structType);

        // For single RGB, try Red/Green/Blue fields
        if (Colors.Count == 1)
        {
            SetByteField(structType, instance, "Red", Colors[0].Color.R);
            SetByteField(structType, instance, "Green", Colors[0].Color.G);
            SetByteField(structType, instance, "Blue", Colors[0].Color.B);
        }
        else // For arrays/complex, use Red0, Green0, Blue0, etc.
        {
            for (int i = 0; i < Colors.Count; i++)
            {
                SetByteField(structType, instance, $"Red{i}", Colors[i].Color.R);
                SetByteField(structType, instance, $"Green{i}", Colors[i].Color.G);
                SetByteField(structType, instance, $"Blue{i}", Colors[i].Color.B);
            }
        }

        // If struct, unbox before returning
        if (structType.IsValueType)
        {
            return Convert.ChangeType(instance, structType);
        }
        return instance;
    }
    ...
    private static void SetByteField(Type type, object obj, string name, byte value)
    {
